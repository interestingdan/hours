...
/*async function dayConsent (dayArray) {
	if (dayArray) {
	dayArray[]
...
async function flexUpdate(userNameArg, userSettingsPlaceHolder) {
	...
	let dayArray = await logDay(startDateObj, endDateObj, userNameArg, userSettingsPlaceHolder.settings);
	let totalScore = 0;
	let dayBuffer =[];
	for (let day of dayArray) {
		...
		rl.question("Enter 'Y' to accept record, 'N' to cancel: ", async function (input) {
			if (input === "Y" || input === "y") {
				...
				console.log("You have rejected the above record, no new records saved, update Rescuetime and try again");
				process.exit();
			}
			})
		}
	if (dayBuffer.length === 0) {
		...
		if (dayBuffer.length === 1) {pluralRecord = "record"} else {pluralRecord = "records"};
		rl.question(`Enter 'Y' to commit ${dayBuffer.length} new day ${pluralRecord}, or N to cancel: `, async function (input) {
			if (input === "Y" || input === "y") {
				for (let day of dayBuffer) {
					userObject.days.push(day);
				};
				await userObject.save().catch((error) => {
					console.error(error);
				});
				...
				console.log("Saving cancelled, no records saved, closing app")
				process.exit();
			}
			rl.close();
		});
